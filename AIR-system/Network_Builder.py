...
class NetworkBuilder:
    ...
    def __init__(self, employers_data: pd.DataFrame, 
        ...
        self.G = None
        # Create a text comparer using LSA
        self.comparer = None
    ...
    def create_network_from_data(self, ) -> nx.MultiDiGraph:
        ...
        for job_id, job_data in self.jobs_data.items():
            ...
            G.add_edge(job_id, job_data['company_id'], weight = GeneralParameter.POSTED_WEIGHT, 
                        edge_type = 'posted')
            G.add_edge(job_data['company_id'], job_id, weight = GeneralParameter.POSTED_WEIGHT, 
                        edge_type = 'posted')
    ...
    def get_lsa(self,) -> LSA.LSA:
        ...
        vocab = LSA.make_vocab(all_texts, min_word_count=10)

        self.lsa = LSA.LSA(vocab, all_documents, GeneralParameter.NUM_REDUCED_FEATURES)
        self.lsa.do_work()
        logger.info(f'Saving LSA comparer to {GeneralParameter.LSA_COMPARER_PATH}')
        with open(GeneralParameter.LSA_COMPARER_PATH, 'wb') as f:
        ...
        for node_name in self.G:
            ...
            if type == 'employer':
                document = [str(node['overview']), str(node['expertise']),
                            str(node['benefit']), str(node['company_name'])]
            elif type == 'job':
    ...
    def add_relations_edges(self, method = 'cosine') -> None:
        ...
        if method == 'knn':
            ...
            num_employer_neighbors = int(GeneralParameter.NEIGHBOR_RATIO * len(employer_node_names))
            ...
            num_job_neighbors = int(GeneralParameter.NEIGHBOR_RATIO * len(job_node_names))
            self.job_knn, job_neighbors = self.get_k_neighbors(job_vectors,
                                        np.arange(len(job_node_names)),
                                        num_job_neighbors)

            num_candidate_neighbors = int(GeneralParameter.NEIGHBOR_RATIO * len(candidate_node_names))
                ...
                for nb_index in nbs[0]:
                    ...
                    if not self.G.has_edge(this_employer, nb_name):
                        self.G.add_edge(this_employer, nb_name,
                                ...
                                weight = GeneralParameter.SIMILAR_WEIGHT)
            ...
            pm_num_neigbors = int(GeneralParameter.PROFILE_MATCHED_NEIHBOR_RATIO * len(candidate_node_names))
                ...
                for nb_index in nbs[0]:
                    ...
                    if not self.G.has_edge(this_candidate, nb_name):
                        self.G.add_edge(this_candidate, nb_name,
                                ...
                                weight = GeneralParameter.PROFILE_MATCH_WEIGHT)
        ...
        if method == 'cosine':
            ...
            for i, id1 in enumerate(employer_node_names):
                ...
                for id2 in employer_node_names[i+1:]:
                    ...
                    sim = 1 - distance.cosine(id1_vector.ravel(), id2_vector.ravel())
                    if sim >= GeneralParameter.COSINE_SIMILARITY_THRESHOLD:
                        self.G.add_edge(id1, id2,
                                    edge_type = 'employer_to_employer',
                                    weight = GeneralParameter.EMPLOYER_TO_EMPLOYER_WEIGHT,
            ...
            for i, id1 in enumerate(job_node_names):
                ...
                for id2 in job_node_names[i+1:]:
                    ...
                    if sim >= GeneralParameter.COSINE_SIMILARITY_THRESHOLD:
                        self.G.add_edge(id1, id2,
                                edge_type = 'job_to_job',
                                weight = GeneralParameter.JOB_TO_JOB_WEIGHT,
                                cosine_similarity = sim)
    ...
    def build(self,) -> None:
        ...
        self.add_relations_edges(method = GeneralParameter.SIMILARITY_METHOD,)
...
if __name__ == '__main__':
    ...
    if os.path.isfile(GeneralParameter.NETWORK_BUILDER_SAVE_PATH):
        logger.info(f'Loading network from {GeneralParameter.NETWORK_BUILDER_SAVE_PATH}')
        with open(GeneralParameter.NETWORK_BUILDER_SAVE_PATH, 'rb') as f:
            network = pickle.load(f)

    else:
        network = NetworkBuilder(GeneralParameter.EMPLOYERS_DATA_PATH,
                            GeneralParameter.JOBS_DATA_PATH,
                            GeneralParameter.CV_DATAPATH)

        network.build()

        logger.info(f'saving network builder object to {GeneralParameter.NETWORK_BUILDER_SAVE_PATH}')

        with open(GeneralParameter.NETWORK_BUILDER_SAVE_PATH, 'wb') as f:
