...
with open('Data\DB\Token_frequency.json', 'r+') as f:
    dist = json.load(f)
RELAX_PARAM = 0.2
new_skill_db = {}
for key in SKILL_DB:
    ...
    skill_category = SKILL_DB[key]['skill_category']
    skill_subcategory = SKILL_DB[key]['skill_subcategory']
    ...
    if skill_len == 2:
        ...
        if last in dist and dist[last] == 1:
            low_surface_form.append(last)
        if start in dist and last in dist and dist[start]/dist[last] < RELAX_PARAM:
...
with open('Data\DB\Final_skill_db.json', 'w', encoding='utf-8') as f:
